...
CONTAINS
    ...
    SUBROUTINE Meltrate(T,NRXF,UT,UTP,NN,Lost)
        INTEGER ::  NN, i
        REAL(KIND=dp) :: ALLOCATABLE :: UTP(:)
        REAL(KIND=dp) :: T
        TYPE(UT_t) :: UT
        TYPE(NRXF_t) :: NRXF
        LOGICAL, ALLOCATABLE :: Lost(:)
        REAL(KIND=dp) :: freezer(3)
        REAL(KIND=dp) :: X,Y,Z
        REAL(KIND=dp) :: maxx,maxy,maxz

        freezer(:) = -500.0

        !Max coords of initial main glacier
        maxz = MAXVAL(NRXF%M(3,:))
        maxy = MAXVAL(NRXF%M(2,:))
        maxx = MAXVAL(NRXF%M(1,:))
        !Removing bottom 20% and front 20% of geometry at t = 1s

        IF (T.EQ.1) THEN
            DO i=1,NN
                !Check whether in melt box
                IF(Lost(i)) CYCLE

                x = NRXF%M(1,I) + UTP(6*I-5)
                y = NRXF%M(2,I) + UTP(6*I-4)
                z = NRXF%M(3,I) + UTP(6*I-3)

                IF(z.LT.0.2*maxz .AND. y.GT.0.8*maxy) THEN
                    Lost(i) = .TRUE.
                END IF

                IF(Lost(i)) THEN
                    PRINT *, myid, " Melted a particle : ",I
                END IF
